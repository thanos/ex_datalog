defmodule ExDatalog.Storage.BackendConformance do
  ...
  defmacro backend_conformance_tests(backend) do
    quote do
      BackendConformance.__init_tests__(unquote(backend))
      BackendConformance.__insert_member_tests__(unquote(backend))
      BackendConformance.__insert_many_tests__(unquote(backend))
      BackendConformance.__stream_tests__(unquote(backend))
      BackendConformance.__size_tests__(unquote(backend))
      BackendConformance.__index_tests__(unquote(backend))
      BackendConformance.__update_index_tests__(unquote(backend))
      BackendConformance.__relations_tests__(unquote(backend))
      BackendConformance.__capabilities_tests__(unquote(backend))
      BackendConformance.__teardown_tests__(unquote(backend))
      BackendConformance.__consistency_tests__(unquote(backend))
    end
  end

  defmacro __init_tests__(backend) do
    quote do
      describe "init/1" do
        test "creates empty storage with all relation schemas" do
          state = unquote(backend).init(BackendConformance.schemas())
  ...
  defmacro __insert_member_tests__(backend) do
    quote do
  ...
  defmacro __insert_many_tests__(backend) do
    quote do
  ...
  defmacro __stream_tests__(backend) do
    quote do
      describe "stream/2" do
        ...
        test "returns deterministically sorted output regardless of insertion order" do
          ...
          state2 = unquote(backend).init(BackendConformance.schemas())
  ...
  defmacro __size_tests__(backend) do
    quote do
      describe "size/2" do
  ...
  defmacro __index_tests__(backend) do
    quote do
  ...
  defmacro __update_index_tests__(backend) do
    quote do
  ...
  defmacro __relations_tests__(backend) do
    quote do
  ...
  defmacro __capabilities_tests__(backend) do
    quote do
  ...
  defmacro __teardown_tests__(backend) do
    quote do
  ...
  defmacro __consistency_tests__(backend) do
    quote do
