defmodule ExDatalog.KnowledgeTest do
  ...
  describe "get/2" do
    test "returns MapSet of tuples for a relation" do
      ...
      assert Knowledge.get(knowledge, "parent") == MapSet.new([{:alice, :bob}])
    end
    ...
    test "returns empty MapSet for unknown relation" do
      ...
      assert Knowledge.get(knowledge, "unknown") == MapSet.new()
    end
  end
  ...
  describe "match/3" do
    test "matches tuples by pattern" do
      ...
      matched = Knowledge.match(knowledge, "parent", [:alice, :_])
    ...
    test "matches with all wildcards" do
      ...
      matched = Knowledge.match(knowledge, "parent", [:_, :_])
    ...
    test "matches with exact values" do
      ...
      matched = Knowledge.match(knowledge, "parent", [:alice, :bob])
      assert MapSet.size(matched) == 1
      assert {:alice, :bob} in matched
    end
  end
  ...
  describe "size/2" do
    test "returns number of tuples" do
      ...
      assert Knowledge.size(knowledge, "parent") == 2
    end
  end
  ...
  describe "relations/1" do
    test "returns sorted list of relation names" do
      ...
      assert Knowledge.relations(knowledge) == ["a", "z"]
    end
  end
end
